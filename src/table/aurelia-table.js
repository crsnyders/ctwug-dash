...
export class AureliaTable {
  ...
  constructor(bindingEngine, fetch) {
    this.formatted = [];
    this.fetch = fetch;
  ...
  tableConfigChanged(newValue, oldValue){
    ...
    this.tableConfig.data = (this.tableConfig.rawdata === 'Promise') ? this.tableConfig.rawdata.then(x=> {return x}) : this.tableConfig.rawdata;
  }
  ...
  applySort(){
    ...
    this.tableConfig.data = this.tableConfig.data.sort(((x,y)=>{
                  var xx = _.get(x,this.tableConfig.orderBy);
                  var yy = _.get(y,this.tableConfig.orderBy);

                  var column = _.head(_.filter(this.tableConfig.columns,x=>{return x.data == this.tableConfig.orderBy}))
                  var customSort = _.get(column,'sortFunction');
                  if(customSort){
    ...
    if(this.tableConfig.orderByDir =='desc'){
      this.tableConfig.data = this.tableConfig.data.reverse();
    }
  }

  applyPaging(){
    var startIndex = this.onFirstPage ? 0 :this.firstItemNumber-1;
    return _.slice(this.tableConfig.data, startIndex,startIndex+this.tableConfig.pageSize)
  }
  ...
  get pages() {
    var numberOfItems = (_.isEmpty(this.tableConfig.filterValue)) ? this.tableConfig.totalItems : _.get(this.tableConfig,'data.length');
...
@computedFrom('tableConfig.totalItems','tableConfig.pageSize','tableConfig.data')
  get pageList(){
    var pagelist =[]
    var pages  = _.parseInt(this.pages);
    if(pages <=7){
      return pages;
    }
    if(this.currentPageNumber >= pages-3){
      return [0,'...',pages-5,pages-4,pages-3,pages-2,pages-1];
    }
    if(this.currentPageNumber <= 3){
      return [0,1,2,3,4,'...',pages-1];
    }
    if(this.currentPageNumber >=4 || this.currentPageNumber <= pages-5){
  ...
  get totalItems() {
    if (this.tableConfig.totalItems === undefined || this.tableConfig.totalItems == 0) {
      this.tableConfig.totalItems = (this.tableConfig.data != null) ? this.tableConfig.data.length : 0;
    }
  ...
  get firstItemNumber() {
    ...
    var firstItem = (this.tableConfig.currentPage * this.tableConfig.pageSize) + 1;
    if (firstItem > this.tableConfig.totalItems) {
      firstItem = 0;
    }
    return firstItem;
  }
  ...
  get lastItemNumber() {
    ...
    if (lastindex > this.tableConfig.totalItems) {
