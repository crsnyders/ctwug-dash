...
function developmentExtender(devEnv: EasyWebpackConfig) {

  return function developmentExtender(this: WebpackConfigWithMetadata): WebpackConfigWithMetadata {
    var cookies = {};
    if (typeof devEnv === 'function') {
      let overlayConfig = devEnv.apply(this) as WebpackConfigWithMetadata
      if (overlayConfig && overlayConfig.devServer) {
        overlayConfig.devServer['headers'] = cookies;
...
let config = generateConfig(
  ...
  ENV === 'test' || ENV === 'development' ?
    developmentExtender(envDev(ENV !== 'test' ? {} : { devtool: 'inline-source-map' })) :
    envProd({ /* devtool: '...' */ }),
